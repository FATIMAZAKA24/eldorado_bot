...
# Tracks Telegram update_ids we've already processed, so if Telegram
# retries delivering the same update (e.g. because we were slow to
# respond), we don't process it - and don't create duplicate listings -
# a second time.
...
def handle_autolist(chat_id, text):
    ...
    """
    try:
        _do_autolist(chat_id, text)
    except Exception as e:
        # Catch-all so a bug never leaves the person staring at silence -
        # they always get *some* message back.
        send_message(chat_id, f"Something went wrong and the process stopped:\n{type(e).__name__}: {e}")


def _do_autolist(chat_id, text):
    ...
    results = []
    for game_key, offer in offers:
        listing = offer_to_listing_data(game_key, offer, markup_percent=20.0)
...
def webhook():
    ...
    if update_id is not None:
        if update_id in processed_update_ids:
            return jsonify({"ok": True})  # already handled this exact update
