...
def fetch_offers(game_name: str, max_pages: int = 1) -> list:
    """
...
def pick_random_offers(game_name: str, count: int) -> list:
    ...
    Returns list of (game_key, offer) tuples."""
    key = game_name.strip().lower()
    offers = fetch_offers(game_name)
    if not offers:
        raise SupplierAPIError(f"No offers currently available for '{game_name}'.")
    picked = random.sample(offers, min(count, len(offers)))
    return [(key, o) for o in picked]
...
def pick_random_mixed_offers(count: int) -> list:
    ...
    for _ in range(count):
        ...
        offers = fetch_offers(game_key)
        if offers:
            picks.append((game_key, random.choice(offers)))
    return picks


def offer_to_listing_data(game_key: str, offer: dict, markup_percent: float = 20.0) -> dict:
    """
    Converts a raw supplier offer into the fields needed to create an
    Eldorado listing, applying the markup. `game_key` is the known,
    reliable game identifier (e.g. "pokemon go") used to fetch this offer -
    NOT parsed from the offer's own text fields, which may have accents
    or different casing than our GAME_LOOKUP keys.
    """
